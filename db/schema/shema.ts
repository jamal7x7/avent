...
import {
  ...
  export const teacherTable = pgTable('teacher', {
    id: text('id').primaryKey(),
    userId: text('user_id').notNull().unique().references(() => userTable.id, { onDelete: 'cascade' }),
  ...
  export const classTeachers = pgTable('class_teachers', {
    ...
    teacherId: text('teacher_id').notNull().references(() => teacherTable.id, { onDelete: 'cascade' }),
  });
  ...
  export const teacherTableRelations = relations(teacherTable, ({ one, many }) => ({
    user: one(userTable, {
