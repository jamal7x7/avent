...
describe("Avatar", () => {
  it("renders image when src is provided", () => {
    render(
      <Avatar src="/test.jpg" alt="Test User" size={40} />
    );
  ...
  it("renders fallback when src is missing", () => {
    ...
    expect(screen.getByLabelText(/Fallback User|User avatar/i)).toBeInTheDocument();
  });

  it("renders fallback when image fails to load", () => {
    render(
      <Avatar src="/broken.jpg" alt="Broken User" size={40} />
    );
    const img = screen.getByAltText("Broken User");
    fireEvent.error(img);
    expect(screen.getByLabelText(/Broken User|User avatar/i)).toBeInTheDocument();
  });
  ...
  it("uses i18n alt text", () => {
    ...
    expect(screen.getByLabelText("الصورة الرمزية للمستخدم")).toBeInTheDocument();
  });
});
